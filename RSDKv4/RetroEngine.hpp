...
// Overridable from the build, because it has to match the data file rather than
// the engine: the opcode table gains and loses entries between revisions (and
// SetPaletteFade even changes arity), so bytecode compiled against one list
// mis-decodes under another. Data from the original Sonic 1 release wants 0.
